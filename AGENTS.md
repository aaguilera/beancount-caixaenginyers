...
- `caixaenginyers.py` — Importer principal. Hereda de `beangulp.importers.csvbase.Importer`. Defineix les columnes del CSV com a atributs de classe (declaratiu), salta 7 línies de capçalera, i usa `csvbase.Amount` amb `subs={",": "."}` per als decimals. Inclou un `Categorizer` per classificar transaccions.
